    ...
    try {
        ...
        setTimeout(() => {
            ...
            if (loadingScreen && loginScreen && startScreen) {
                ...
                console.error('Required elements not found:', { loadingScreen, loginScreen, startScreen });
                showNotification('Game failed to initialize: Missing UI elements.');
            }
        }, 1000);
    } catch (error) {
        console.error('Error initializing game:', error.message, error.stack);
        showNotification('Failed to initialize game. Check console for details.');
        throw error; // Biar error tetap muncul di console
    }
}
